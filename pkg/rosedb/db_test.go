...
func TestDB_Put_Normal(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		assert.Nil(t, err)
		err = db.Put(utils.GetTestKey(rand.Int()), utils.RandomValue(wal.KB))
		assert.Nil(t, err)
		err = db.Put(utils.GetTestKey(rand.Int()), utils.RandomValue(5*wal.KB))
		assert.Nil(t, err)
	}
...
func TestDB_Get_Normal(t *testing.T) {
	...
	generateData(t, db, 200, 300, wal.KB)
	for i := 200; i < 300; i++ {
		val, err := db.Get(utils.GetTestKey(i))
		assert.Nil(t, err)
		assert.Equal(t, len(val), len(utils.RandomValue(wal.KB)))
	}
	generateData(t, db, 400, 500, 4*wal.KB)
	for i := 400; i < 500; i++ {
		val, err := db.Get(utils.GetTestKey(i))
		assert.Nil(t, err)
		assert.Equal(t, len(val), len(utils.RandomValue(4*wal.KB)))
	}
}
...
func TestDB_Auto_Merge(t *testing.T) {
	...
	for i := 0; i < 2000; i++ {
		...
		assert.Nil(t, err)
		err = db.Put(utils.GetTestKey(rand.Int()), utils.RandomValue(2*wal.KB))
		assert.Nil(t, err)
